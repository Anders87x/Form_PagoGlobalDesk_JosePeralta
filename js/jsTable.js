var csmJsTable = (function ()
{
    ...
    var convertToDecimal = function (pValue, pDecimales)
    {
        ...
        pValue = pValue.toFixed(pDecimales);
        var splitStr = pValue.split('.');
    ...
    var pintarFila = function (r)
    {
        if (selectIndexRow === -1)
        {
            r.style.cssText = "cursor:pointer; outline:#25476a solid 1px !important";
            selectIndexRow = r.sectionRowIndex;
        }
        else
        {
            let oldTr = r.parentNode.childNodes[selectIndexRow];
            oldTr.style.outlineColor = "";
            oldTr.style.outlineStyle = "";
            oldTr.style.outlineWidth = "";
    ...
    function animateShowText(pText, pThis)
    {
        var len = pText.length;
        var text = pThis.textContent.replace("...", "");
        var letra = text.length;
        pThis.textContent = text;
        var intervalo = window.setInterval(frame, 30);
        function frame()
        {
            if (letra < len)
            {
                pThis.textContent = pThis.textContent + pText[letra];
                letra ++;
            }
            else if (letra >= len)
            {
    ...
    var fillData = function (pConfig)
    {
        ...
        for (i = 0; i < cuentaRow; i++)
        {
            ...
            for (j = 0; j < cuentaCols; j++)
            {
                ...
                if (col.isHtmlText === undefined) col.isHtmlText = false;
                ...
                if (col.typePresentacion === "cell")
                {
                    ...
                    else if (col.dataType === "decimal") newValor = convertToDecimal(valor, col.cantDecs === undefined ? 2 : col.cantDecs);

                    else if (col.dataType === "date") newValor = strDateTime(valor,false);

                    else if (col.dataType === "dateTime") newValor = strDateTime(valor, true);

                    if (col.isHtmlText === false)
                    {
                        if (col.dataType === "string" && col.trimText === true && col.trimLen > 0) {
                            oTdCell.title = newValor;
                            var lenString = newValor.length;
                            if (lenString > col.trimLen) oTdCell.textContent = newValor.toString().substr(0, col.trimLen) + "...";
                            else oTdCell.textContent = newValor;

                            oTdCell.onfocus = function () {
                                var title = this.title;
                                var text = this.textContent;
                                this.textContent = title;
                                //animateShowText(title, this);
                                this.title = text;
                            }
                            oTdCell.onblur = function () {
                                var title = this.title;
                                var text = this.textContent;
                                this.textContent = title;
                                this.title = text;
                    ...
                    else if (col.isHtmlText === true)
                    {
                        if (col.dataType === "string" && col.trimText === true && col.trimLen > 0) {
                            oTdCell.title = newValor;
                            var lenString = newValor.length;
                            if (lenString > col.trimLen) oTdCell.innerHTML = newValor.toString().substr(0, col.trimLen) + "...";
                            else oTdCell.innerHTML = newValor;
                ...
                else if (col.typePresentacion === "template")
                {
                    ...
                    for (var k = 0; k < lenControls; k++)
                    {
                        ...
                        if (ctl.ele === "input" && (ctl.type === "text" || ctl.type === "number"))
                        {
                            ...
                            if (ctl.list !== undefined) element.setAttribute("list", ctl.list);
                            ...
                            if (ctl.maxLength !== undefined) element.setAttribute("maxlength", ctl.maxLength);
                        }
                        ...
                        else if (ctl.ele === "input" && (ctl.type === "button"))
                        {
                            ...
                            {
                                element.value = ctl.value;
                                element.setAttribute("data-tag", valor);
                            }
                            if (ctl.style !== undefined && ctl.style !== "") element.setAttribute("style", ctl.style);
                            if (ctl.className !== undefined && ctl.className !== "")
                                {element.className = ctl.className;}
                            else
                            {
                                element.className += " cl_" + valor;
                            }
                        ...
                        else if (ctl.ele === "span") {
                            element.id = ctl.id + "_" + i.toString();
                            if (ctl.bind) element.textContent = valor;
                            else element.textContent = ctl.value;
                            if (ctl.style !== undefined && ctl.style !== "") element.setAttribute("style", ctl.style);
                            if (ctl.className !== undefined && ctl.className !== "") element.className = ctl.className; else element.className = "";
                        }
                        else if (ctl.ele === "i")
                        {
                            element.id = ctl.id + "_" + i.toString();
                            if (ctl.bind) element.textContent = valor;
                            else element.title = ctl.value;
                            element.className = ctl.clssBtn;
                        ...
                        if (ctl.OnBlur !== undefined)
                        {
                            element.onblur = function (e) {
                                var xEle = this;
                                var arr = xEle.getAttribute("data-Ctl").split("|");
                                pConfig.columns[Number(arr[1])].controls[Number(arr[0])].OnBlur(xEle, e);
                            }
                        }
            ...
            if (pConfig.dataInfo !== undefined)
            {
                var dataInfoArr = pConfig.dataInfo.split(",");
                var longX = dataInfoArr.length;
                var info = row[dataInfoArr[0]];
                for (var n = 1; n < longX; n++)
                {
                    info += "|" + row[dataInfoArr[n]];
                }

                oTrRow.setAttribute("data-Info", info);
            }

            if (pConfig.onInsertRow !== undefined) pConfig.onInsertRow(oTrRow);
    ...
    var fillTfooter = function (pConfig)
    {
        ...
        oBtnModel.className = "btn btn-primary btn-xs";
        oBtnModel.setAttribute("style", "width:96% ;text-align:center;font-weight: bold;cursor: pointer; font-size: 10px; border: none; border-radius: 0;");
        ...
        oInputText.className = "btn btn-outline btn-default btn-xs";
        oInputText.setAttribute("style", "border:solid 1px #ccc; width:96%; text-align:center; font-size: 10px; border: none; border-radius: 0;");
        ...
        divLeyenda.setAttribute("style", "width:99%;text-align:center;cursor: pointer; font-size: 12px;");
    ...
    var fillFiltros = function (pConfig)
    {
        ...
        if (pConfig.defaultFieldSearch === undefined)
        {
            for (i = 0; i < cuentaCols; i++)
            {
                ...
                option.textContent = col.titleFilter || col.title;
    ...
    var orderData = function (pCol, pConfig) {
        debugger;
        var colSort = pCol.getAttribute("data-sort");
        var colTypeSort = pCol.getAttribute("data-TypeSort");
        var colName = colSort!== null ? colSort : pCol.getAttribute("data-Column");
        var typeCol = colTypeSort !== null ? colTypeSort : pCol.getAttribute("data-Type");
    ...
    function browserKeyTable(pKey, pTarget)
    {
        ...
        if (pKey === 39)
        {
            ...
            //ele.focus();
            if (ele !== null && ele != undefined) ele.focus();
        }
        else if (pKey === 37)
        {
            parent = pTarget.parentNode;
    ...
    var fillTable = function (pConfig)
    {
        ...
        oTrHead.style.cssText = "background-color:#25476a; color:white;";
        var oThModel = dom.createElement('th');
        oThModel.setAttribute("style", "padding:4px 0 4px 0 !important; text-align: center; cursor:pointer; font-size: 11px; color:white;");
        ...
        for (i = 0; i < cuentaCols; i++)
        {
            ...
            if (col.dataType === undefined) col.dataType = "string";
            if (col.dataSort !== undefined) oTh.setAttribute("data-sort", col.dataSort);
            if (col.dataTypeSort !== undefined) oTh.setAttribute("data-TypeSort", col.dataTypeSort);
            oTh.setAttribute("data-Type", col.dataType);
            oTh.setAttribute("data-OrderType", "-1");
            oTh.setAttribute("data-Title", col.title);
            if (col.colWidth !== undefined)
            {
                oTh.classList.add("col-md-" + col.colWidth);
                oTh.classList.add("col-xs-" + col.colWidth);
            }
            if (col.classCol !== undefined) oTh.classList.add(col.classCol);
        ...
        if (len > 0)
        {
            ...
            if (pConfig.OnAfterLoad2 !== undefined)
                pConfig.OnAfterLoad2(idTable);
            if (pConfig.OnAfterLoad !== undefined)
                pConfig.OnAfterLoad(idTable);
    ...
    var excuteUpdate = function (pData)
    {
        ...
        {
            itemData = pData[i];
            campo = itemData.campo;
            value = itemData.value;
            arrData[itemData.index][campo] = value;

            if (itemData.tr !== undefined)
            {
                itemData.dataTr[campo] = value;
                itemData.tr.setAttribute("data-keys", JSON.stringify(itemData.dataTr));
            }
        }
        len = arrData.length;
        arrFiltros.length = 0;
        for (let j = 0; j < len; j++) arrFiltros.push(arrData[j]);
    }
    ...
    var goToPageData = function (pPage)
    {
        var valor = Number(pPage);
        if (!isNaN(valor))
        {
            if (valor >= 1 && valor <= gTotalPage)
            {
                if (gConfig.OnBeforePage !== undefined) gConfig.OnBeforePage();
                gCurrentpage = valor;
                fillTBody(gConfig);
                pintarLeyenda(gConfig.rowForPage);

                if (gConfig.OnAfterPage !== undefined)
                {
                    gConfig.OnAfterPage();
                }
                paintRow(currentIndexRow);
    ...
    return {
        ...
        goToPageData: goToPageData,
        getIdInputPagination: function () { return idInputText; },
        getLastNumberPage: function () { return gTotalPage },
        getIdTxtFiltro: function () { return idTxtFiltro; },
        getIdTbody: function () { return idTbody; },
        getIdSelectFilter: function () { return idSelectFilter; }
    }
});